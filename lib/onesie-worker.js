  ...
  const handleOnsieRequest = async (taskData) => {
    ...
    for (let attempt = 0; ; attempt++) {
      ...
      try {
        let result
        if (metadataOnly) {
          result = await getBasicInfoMetadata(url, format, requestInnertube, tvConfig, logger)
        } else {
          result = await onesieFormatRequest(url, format, requestInnertube, tvConfig, returnRedirectUrl, logger)
        }
        ...
        if (err instanceof SabrOnlySessionError || err instanceof StreamingDataUnavailableSessionError) {
          ...
          if (attempt < maxSessionRerolls) {
            logger.warn({ url, attempt: attempt + 1, maxSessionRerolls, errorCode: err.code }, 'YouTube session did not return usable streaming data — re-rolling Innertube visitor session')
            continue
          }
        }
