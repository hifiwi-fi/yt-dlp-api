...
test('RedisICache rejects pending and new operations when its IPC channel closes', async () => {
  const channelName = `redis-icache-test-${randomUUID()}`
  const cache = new RedisICache(new BroadcastChannel(channelName))
  const pendingGet = cache.get('pending')

  cache.close()

  await assert.rejects(pendingGet, /Redis cache IPC channel closed/)
  await assert.rejects(cache.get('new'), /Redis cache IPC channel closed/)
})

test('RedisICache removes pending state when sending an IPC request fails', async () => {
  const channelName = `redis-icache-test-${randomUUID()}`
  const channel = new BroadcastChannel(channelName)
  channel.postMessage = () => {
    throw new Error('IPC send failed')
  }
  const cache = new RedisICache(channel)

  await assert.rejects(cache.get('key'), /IPC send failed/)
  cache.close()

  // Give the test runner a turn to detect an unhandled orphaned rejection.
  await sleep(0)
})
