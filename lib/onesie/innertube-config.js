 ...
 * @import { Innertube as InnertubeType, Types } from 'youtubei.js'
 */
 ...
 * @prop {Types.ICache} [cache] - Custom cache implementation (ICache interface)
 * @prop {string | undefined} [playerId] - Specific player ID to use (e.g., '0004de42')
 */
...
export class InnertubeConfig {
  ...
  async #refreshInnertube () {
    ...
    try {
      this.inFlightRefresh = createInnertubeInstance(this.cache, this.playerId)
...
/**
 * Creates a new Innertube instance with custom cache or UniversalCache fallback.
 * @param {Types.ICache} [customCache] - Custom cache implementation (ICache interface)
 * @param {string} [playerId] - Specific player ID to use
 * @return {Promise<import('youtubei.js').Innertube>}
 */
async function createInnertubeInstance (customCache, playerId) {
  const cache = customCache || new UniversalCache(true)
  /** @type { Types.InnerTubeConfig } */
  const config = { cache }
  // Only add player_id if it's explicitly provided and not null/undefined
  if (playerId != null) {
