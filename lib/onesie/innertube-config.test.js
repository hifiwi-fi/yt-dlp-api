...
test('InnertubeConfig - creates instance with custom refresh time', async (_t) => {
  ...
  const config = new InnertubeConfig({ refreshMs: customRefreshMs, playerId: defaultPlayerId })
...
test('InnertubeConfig - getInnertube creates new instance on first call', async (_t) => {
  const config = new InnertubeConfig({ playerId: defaultPlayerId })
...
test('InnertubeConfig - getInnertube returns cached instance before refresh time', async (_t) => {
  const config = new InnertubeConfig({ refreshMs: 1000 * 60 * 60, playerId: defaultPlayerId }) // 1 hour
...
test('InnertubeConfig - refreshes instance after refresh time', async (_t) => {
  ...
  const config = new InnertubeConfig({ refreshMs: 100, playerId: defaultPlayerId }) // 100ms
