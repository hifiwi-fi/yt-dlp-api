...
import { onesieFormatRequest, getBasicInfoMetadata, extractYouTubeVideoId } from './index.js'
...
// Live YouTube extraction does not pass reliably in CI.
test('onesieFormatRequest test', { todo: process.env.CI }, async (_t) => {
  const innertube = await Innertube.create({ /* player_id: defaultPlayerId , */ retrieve_innertube_config: true })
  const tvConfigInstance = new YouTubeTVClientConfig()

  const result = await onesieFormatRequest(videoUrl, 'video', innertube, tvConfigInstance, true, logger)
