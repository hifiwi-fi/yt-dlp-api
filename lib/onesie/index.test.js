...
test('onesieFormatRequest test', async (_t) => {
  const innertubeConfig = new InnertubeConfig({ playerId: defaultPlayerId })
  const tvConfigInstance = new YouTubeTVClientConfig()

  const result = await onesieFormatRequest(videoUrl, 'video', innertubeConfig, tvConfigInstance)
