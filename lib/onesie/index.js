...
/**
 * YouTube A/B-buckets some fresh visitor sessions into SABR-only streaming:
 * the player response ships no progressive formats (`streamingData.formats`
 * absent) and no per-format URLs, only `serverAbrStreamingUrl`. The bucket is
 * sticky for the visitor session — recovery requires re-rolling the session.
 */
export class SabrOnlySessionError extends Error {
  code = 'SABR_ONLY_SESSION'

  constructor () {
    super('YouTube served a SABR-only session: no progressive formats or format URLs available')
  }
}

/**
 * YouTube occasionally marks a video as playable while withholding all
 * streaming data for a visitor session. A fresh visitor session can recover.
 */
export class StreamingDataUnavailableSessionError extends Error {
  code = 'STREAMING_DATA_UNAVAILABLE_SESSION'

  constructor (cause) {
    super('YouTube did not return streaming data for this session', { cause })
  }
}
 ...
 */
export async function onesieRequest (youtubeUrl, innertube, tvConfigInstance, poToken, log) {
  ...
  const rawMetadata = await getRawMetadata({ innertube, poToken, clientConfig, videoId, log })
...
async function prepareOnesieRequest ({
  ...
  // Change or remove these if you want to use a different client. The upstream
  // googlevideo onesie-request example uses TVHTML5, but that client returns a
  // limited player response (no videoDetails metadata), so we stay on WEB.
 ...
 * @param  {ClientConfig} params.clientConfig
 * @param  {InnerTube} params.innertube
 * @param {FastifyBaseLogger} params.log
 */
async function getRawMetadata ({ innertube, videoId, poToken, clientConfig, log }) {
  ...
  function handleSabrError (part) {
    ...
    log.error({ sabrError: error }, 'YouTube returned a SABR error')
  }
  ...
  function handleOnesieData (part) {
    ...
    if (onesie.length > 0) {
      ...
      log.warn({}, 'Received ONESIE_DATA without a preceding ONESIE_HEADER')
    }
  }
...
export async function processMetadata (metadataRaw, format, innertube) {
  ...
  let formatData
  if (!is_upcoming) {
    const streamingData = metadataRaw?.streamingData
    if (!info.basic_info.is_live && !streamingData?.formats?.length && streamingData?.serverAbrStreamingUrl) {
      throw new SabrOnlySessionError()
    }

    try {
      formatData = info.chooseFormat(formatSelector[format])
    } catch (err) {
      if (err instanceof Error && err.message === 'Streaming data not available') {
        throw new StreamingDataUnavailableSessionError(err)
      }
      throw err
    }
  }
...
export async function getBasicInfoMetadata (youtubeUrl, format, innertube, tvConfig, log) {
  const metadataRaw = await onesieRequest(youtubeUrl, innertube, tvConfig, undefined, log)
  ...
  if (!is_upcoming) {
    try {
      ...
      log.warn({ err }, 'chooseFormat failed during discovery; filesize_approx will be null')
    }
  }
...
export async function onesieFormatRequest (youtubeUrl, format, innertube, tvConfig, livenUrl = true, log) {
  ...
  if (livenUrl && url) {
    ...
    if (adWaitMs > 0) {
      ...
      log.info({ adWaitMs }, `Sleeping ${waitSec}s as required by pre-roll ads`)
        ...
        .compose(interceptors.retry({
          ...
          retry (err, { state, opts }, callback) {
            ...
            if (attempt >= maxRetries) {
              log.warn({ err, attempt, maxRetries }, 'HEAD request retry limit exceeded')
              callback(err)
              return
            }
            ...
            if (shouldRetry) {
              log.warn({ err, status, attempt: attempt + 1, maxRetries }, 'Retrying HEAD request for YouTube media URL')
      ...
      const redirectLocation = typeof res.headers.location === 'string'
      ...
      log.info({
        ...
        validationStatusCode: res.statusCode,
        redirectHost: redirectLocation
          ? new URL(redirectLocation, url).hostname
          : null,
      }, 'YouTube media URL validation completed')
