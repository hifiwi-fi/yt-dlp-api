...
async function prepareOnesieRequest ({
  ...
  // Will determine the shape of the returned metadata
...
async function getRawMetadata ({ innertube, videoId, poToken, clientConfig }) {
  ...
  const onesieInnertubeResponse = Protos.OnesieInnertubeResponse.decode(decryptedData)

  if (onesieInnertubeResponse.proxyStatus !== 1) { throw new Error('Proxy status not OK') }
  if (onesieInnertubeResponse.status !== 200) { throw new Error('Status not OK') }
